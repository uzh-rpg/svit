...
import models, selective_vit
import utils
from helpers import speed_test, get_macs

from tensorboardX import SummaryWriter
import wandb
from change_name_of_state_dict import change_key_name_from_timm_to_pytorch, change_key_name_from_timm_to_evo
import warnings
...
def get_args_parser():
    ...
    parser.add_argument('--exp_name', default='', type=str, help='name of the experiment to be logged in wandb')

    # sparse training parameters
    parser.add_argument('--ratio-loss-lambda', type=float, default=None, help='If not zero, use ratio loss')
    parser.add_argument('--ratio-per-sample', default=False, action='store_true')
    parser.add_argument('--change-key-name-from-timm-to-pytorch', default=False, action='store_true',
                        help='change key names for state_dict from timm version to pytorch version')
    parser.add_argument('--change-key-name-from-timm-to-evo', default=False, action='store_true')
    return parser
...
def main(args):
    ...
    wandb.init(project="Imagenet1k&Cifar10,100 classification", entity='team52', config=args)
    if args.exp_name:
    ...
    if args.ratio_loss_lambda:
        model.ratio_loss = True
    if args.ratio_per_sample:
        model.ratio_per_sample = True
    ...
    if args.finetune:
        ...
        if args.change_key_name_from_timm_to_pytorch:
            checkpoint_model = change_key_name_from_timm_to_pytorch(checkpoint_model)
        elif args.change_key_name_from_timm_to_evo:
            checkpoint_model = change_key_name_from_timm_to_evo(checkpoint_model)
        incompatible_keys = model.load_state_dict(checkpoint_model, strict=False)
        print(f'missing_keys:{incompatible_keys.missing_keys}\n'
    ...
    if args.eval:
        from token_stat import selector_hist, selector_depth, raw_selector_hist
        from plot_distribution import plot_distribution
        selector_depth[0][:] = 0
        model.visualize = True
        model.statistics = True
        test_stats = evaluate(data_loader_val, model, device, args=args)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        plot_distribution(selector_hist, selector_depth, raw_selector_hist, len(dataset_val))
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % test_interval == 0 or epoch == args.epochs - 1:
            ...
            test_stats1 = {f'eval_{k}': v for k, v in test_stats.items()}
        else:
            test_stats1 = {}
