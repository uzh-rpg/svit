...
from token_stat import select_stat, rescale_stat, selector_record, attn_weights_record, weighting_loss_record
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            if args.ratio_loss_lambda:
                outputs, ratio_loss = model(samples)
        ...
        if args.ratio_loss_lambda:
            loss = loss + args.ratio_loss_lambda * ratio_loss
        ...
        if args.ratio_loss_lambda:
            metric_logger.update(ratio_loss=ratio_loss.item())
...
def evaluate(data_loader, model, device, keep_rate=None, args=None):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            if args.ratio_loss_lambda:
                output, ratio_loss = model(images)
        ...
        if args.ratio_loss_lambda:
            metric_logger.meters['ratio_loss'].update(ratio_loss.item())
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    ...
    for i in range(12):
        print(f'Layer {i + 1} \t {select_stat[i].avg if select_stat[i].avg != 0 else all}'
              f'\t {rescale_stat[i].avg if rescale_stat[i].avg != 0 else none}')
        select_stat[i].reset()
        rescale_stat[i].reset()
    if weighting_loss_record[0] is not None:
        print(f'weighting: {weighting_loss_record[0]}')
    selector_record.clear()
    attn_weights_record.clear()
